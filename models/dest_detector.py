...
from .dest_module import Serialization, ISSMDecoderLayer
...
class DestDetector(nn.Module):
    ...
    def __init__(self, num_class, num_heading_bin, num_size_cluster, mean_size_arr,
                 ...
                 self_position_embedding='xyz_learned', issm_position_embedding='xyz_learned',
                 size_cls_agnostic=False, use_biscan=False):
        super().__init__()
        ...
        self.issm_position_embedding = issm_position_embedding
        ...
        # Position Embedding for ISSM Scan
        if self.issm_position_embedding == 'none':
            self.decoder_issm_posembeds = [None for i in range(num_decoder_layers)]
        elif self.issm_position_embedding == 'xyz_learned':
            self.decoder_issm_posembeds = nn.ModuleList()
            for i in range(self.num_decoder_layers):
                self.decoder_issm_posembeds.append(PositionEmbeddingLearned(3, 288))
        else:
            raise NotImplementedError(f"issm_position_embedding not supported {self.issm_position_embedding}")

        # Transformer decoder layers
        self.decoder = nn.ModuleList()
        self.serialization_key = Serialization (order=["zyx", "zxy", "xzy", "xyz", "yxz", "yzx", "zyx", "zxy", "xzy", "xyz", "yxz", "yzx"])
        for i in range(self.num_decoder_layers):
            self.decoder.append(
                ISSMDecoderLayer(
                    288, nhead, dim_feedforward, dropout, activation, num_proposal=num_proposal,
                    self_posembed=self.decoder_self_posembeds[i],
                    issm_posembed=self.decoder_issm_posembeds[i], 
                    use_biscan=use_biscan,
    ...
    def forward(self, inputs):
        ...
        # Position Embedding for ISSM
        if self.issm_position_embedding == 'none':
            key_pos = None
        elif self.issm_position_embedding in ['xyz_learned']:
            key_pos = points_xyz
        else:
            raise NotImplementedError(f"issm_position_embedding not supported {self.issm_position_embedding}")

        self.serialization_key(key_pos)
        for i in range(self.num_decoder_layers):
            ...
            key_seri = self.serialization_key.reorder_points(key.transpose(1,2), id=i).transpose(1,2)
            query, key_seri = self.decoder[i](query, key_seri, query_pos, key_pos_seri)
            if not self.decoder[i].last_layer:
                key = self.serialization_key.reversed_reorder_points(key_seri.transpose(1,2)).transpose(1,2)
