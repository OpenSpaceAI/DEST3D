...
class PredictHead(nn.Module):
    def __init__(self, num_class, num_heading_bin, num_size_cluster,
        ...
        if num_heading_bin == 1:
            self.class2angle = self.class2angle_scannet
        else:
            self.class2angle = self.class2angle_sunrgbd

    def class2angle_scannet(self, pred_cls, residual, to_label_format=True):
        ''' Inverse function to angle2class.
        
        As ScanNet only has axis-alined boxes so angles are always 0. '''
        return torch.zeros_like(pred_cls)

    def class2angle_sunrgbd(self, pred_cls, residual, to_label_format=True):
        ''' Inverse function to angle2class '''
        angle_per_class = 2 * torch.pi / float(self.num_heading_bin)
    ...
    def forward_pred_obj(self, net, end_points, prefix=''):
        """
        Args:
            features: (B,C,num_proposal)
        Returns:
    ...
    def forward_pred_box(self, net, base_xyz, end_points, prefix=''):
        """
        Args:
            features: (B,C,num_proposal)
        Returns:
    ...
    def inference(self, features, base_xyz):
        """
        Args:
            features: (B,C,num_proposal)
        Returns:
        ...
        heading_residuals_normalized = self.heading_residual_head(net).transpose(2, 1)
        heading_residuals = heading_residuals_normalized * (np.pi / self.num_heading_bin)
        heading_cls = torch.argmax(heading_scores, -1)
        heading_angle = self.class2angle(heading_cls, heading_residuals.unsqueeze(-1))
        ...
        end_points['heading_angle'] = heading_angle
        return end_points
