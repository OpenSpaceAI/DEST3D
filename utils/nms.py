...
def lsm_3d_faster_samecls(boxes, overlap_threshold, old_type=False):
    x1 = boxes[:, 0]
    y1 = boxes[:, 1]
    z1 = boxes[:, 2]
    x2 = boxes[:, 3]
    ...
    while (I.size != 0):
        ...
        rm_inds = np.where(o > overlap_threshold)[0]
        I = np.delete(I, np.concatenate(([last - 1], rm_inds)))

        if rm_inds.size < 4:
            pick.extend(rm_inds.tolist())
        else:
            pick.extend(rm_inds[:4].tolist())
